"use client";
import React from "react";
import Link from "next/link";
import { useState } from "react";
import Logout from "./Logout";
import { CONSUMER, BUSINESS, EMPLOYEE } from "@/lib/utils";
import { buttonVariants } from "./ui/button";

const NavbarContent = ({ session }) => {
  const [open, setOpen] = useState(false);
  const [openMobile, setOpenMobile] = useState(false);

  const handleClickRight = () => {
    setOpen(!open);
  };

  const handleClickLeftMobile = () => {
    console.log("click", openMobile);
    setOpenMobile(!openMobile);
  };
  return (
    <>
      <nav className="relative">
        <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
          <div className="relative flex h-16 items-center justify-between">
            {/* hamburger menu mobile */}
            <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
              <button
                onClick={handleClickLeftMobile}
                type="button"
                className="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white"
                aria-controls="mobile-menu"
                aria-expanded="false"
              >
                <span className="absolute -inset-0.5"></span>
                <span className="sr-only">Open main menu</span>

                <svg
                  className="block h-6 w-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                  aria-hidden="true"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"
                  />
                </svg>

                <svg
                  className="hidden h-6 w-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                  aria-hidden="true"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M6 18L18 6M6 6l12 12"
                  />
                </svg>
              </button>
            </div>
            <div className="flex flex-1 items-center justify-center sm:items-stretch sm:justify-start">
              <Link
                href="/"
                className="flex flex-shrink-0 items-center font-bold"
              >
                FNTASTIC.
              </Link>
              <div className="hidden sm:ml-6 sm:block">
                {/* Menu principal, dashboard, etc /dashboard/business/employees */}
                {session?.user && (
                  <div className="flex space-x-4">
                    <Link
                      href="/dashboard"
                      className="text-gray-500 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"
                    >
                      Tu negocio
                    </Link>
                    <Link
                      aria-current="page"
                      href="/dashboard/business/reservations"
                      className="text-gray-500 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"
                    >
                      Reservas
                    </Link>
                    <Link
                      href="/dashboard/business/services"
                      className="text-gray-500 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"
                    >
                      Servicios
                    </Link>
                    <Link
                      href="/dashboard/business/working-hours"
                      className="text-gray-500 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"
                    >
                      Horarios
                    </Link>
                    <Link
                      href="/dashboard/business/employees"
                      className="text-gray-500 hover:bg-gray-700 hover:text-white rounded-md px-3 py-2 text-sm font-medium"
                    >
                      Empleados
                    </Link>
                  </div>
                )}
              </div>
            </div>
            {/*Menu derecha  */}
            {/* <NavbarRight session={session} /> */}

            {session ? (
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                <button
                  type="button"
                  className="relative rounded-full p-1 text-gray-400 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
                >
                  <span className="absolute -inset-1.5"></span>
                  <span className="sr-only">View notifications</span>
                  <svg
                    className="h-6 w-6"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth="1.5"
                    stroke="currentColor"
                    aria-hidden="true"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0"
                    />
                  </svg>
                </button>

                <div className="relative ml-3">
                  <div>
                    <button
                      onClick={handleClickRight}
                      type="button"
                      className="relative flex rounded-full bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800"
                      id="user-menu-button"
                      aria-expanded="false"
                      aria-haspopup="true"
                    >
                      <span className="absolute -inset-1.5"></span>
                      <span className="sr-only">Open user menu</span>
                      <img
                        className="h-8 w-8 rounded-full"
                        src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80"
                        alt=""
                      />
                    </button>
                  </div>

                  <div
                    className={
                      open
                        ? "absolute right-0 z-10 mt-2 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none w-40"
                        : "hidden absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
                    }
                    role="menu"
                    aria-orientation="vertical"
                    aria-labelledby="user-menu-button"
                    tabIndex="-1"
                  >
                    <Link
                      href="/dashboard"
                      className="block px-4 py-2 text-sm text-gray-700"
                      role="menuitem"
                      tabIndex="-1"
                      id="user-menu-item-0"
                      onClick={handleClickRight}
                    >
                      Tu negocio
                    </Link>

                    <Logout onClick={handleClickRight} />
                  </div>
                </div>
              </div>
            ) : (
              <div className="hidden items-center space-x-4 sm:flex">
                <Link
                  href={`/auth/signup/${BUSINESS}`}
                  className={buttonVariants({
                    variant: "ghost",
                    size: "sm",
                  })}
                >
                  Regristrar negocio
                </Link>
                <Link
                  href="/auth/login"
                  className={buttonVariants({
                    variant: "ghost",
                    size: "sm",
                  })}
                >
                  Iniciar sesión
                </Link>
                <Link
                  href={`/auth/signup/${CONSUMER}`}
                  className={buttonVariants({
                    size: "sm",
                  })}
                >
                  Crear cuenta
                </Link>
              </div>
            )}
          </div>
        </div>

        {/* Menu izqueirda mobile */}

        <div className={openMobile ? "sm:hidden" : "hidden"} id="mobile-menu">
          <div className="space-y-1 px-2 pb-3 pt-2 absolute z-10 bg-white w-full">
            <Link
              onClick={() => setOpenMobile(false)}
              href="/dashboard"
              className="text-gray-500 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium"
            >
              Tu negocio
            </Link>
            <Link
              onClick={() => setOpenMobile(false)}
              aria-current="page"
              href="/dashboard/business/reservations"
              className="text-gray-500 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium"
            >
              Reservas
            </Link>
            <Link
              onClick={() => setOpenMobile(false)}
              href="/dashboard/business/services"
              className="text-gray-500 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium"
            >
              Servicios
            </Link>
            <Link
              onClick={() => setOpenMobile(false)}
              href="/dashboard/business/working-hours"
              className="text-gray-500 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium"
            >
              Horarios
            </Link>
            <Link
              onClick={() => setOpenMobile(false)}
              href="/dashboard/business/employees"
              className="text-gray-500 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 text-base font-medium"
            >
              Empleados
            </Link>
          </div>
        </div>
      </nav>
    </>
  );
};

export default NavbarContent;
